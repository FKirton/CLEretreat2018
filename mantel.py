...
def read_file(path):
    ...
    complete_split = [element.split(",") for element in split_data]
    first_column = [element[0] for element in complete_split]
    second_column = [element[1] for element in complete_split]
...
def levenshtein_distance(s1, s2):
    ...
    '''
    if len(s1) > len(s2):       # makes sure that s1 is the longer list (the longer word)
        s1,s2 = s2,s1
    distances = range(len(s1) + 1)  # range over longer word
